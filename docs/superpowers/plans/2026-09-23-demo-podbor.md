...
- Рабочий диапазон Q кривой = реально нарисованный жирный участок кривой (`qRangeGraph`), если
  он есть, иначе диапазон из таблицы каталога; за него квадратичную аппроксимацию не
  экстраполировать. Валидация оцифровки: P в точках `qRangeGraph` сравнивается с табличным
  диапазоном давлений (допуск 8%).
...
- [ ] **Step 1-7:** повторить шаги Task 2 (15 кривых вместо 10 — по 3 на каждый из 5 диаметров;
  ...
  **с теми же решениями, что в Task 3**: использовать закоммиченный
  ...
  README первым), тест — копия закоммиченного `scripts/test-data-2-5.js` (сравнение P в точках
  `qRangeGraph`, а не табличного Qmax), поля `qRangeGraph`/`pRangeGraph` обязательны, проверить K для мощностей
