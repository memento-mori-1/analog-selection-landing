...
function run() {
  const data = JSON.parse(fs.readFileSync(__dirname + '/digitize/data_3_15.json', 'utf8'));
  let failed = 0;
  for (const dia of data.diameters) {
    for (const curve of dia.curves) {
      ...
      // Каталожные границы Q в таблице округлены и одинаковы для всех диаметров (0,76-1,90/2,00 / 1,60-4,00),
      // а жирная кривая на графике заканчивается чуть раньше или позже. Квадратичную аппроксимацию
